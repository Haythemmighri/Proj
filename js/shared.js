...
function checkout() {
    ...
    invoice += `Heure: ${new Date().toLocaleTimeString('fr-FR')}\n`;
    invoice += '------------------------\n\n';

    cart.forEach(item => {
        const itemTotal = item.price * item.quantity;
        invoice += `${item.name}\n`;
        invoice += `  Quantité: ${item.quantity} x ${item.price.toFixed(2)} € = ${itemTotal.toFixed(2)} €\n`;
    });

    invoice += '\n------------------------\n';
    invoice += `TOTAL: ${total.toFixed(2)} €\n\n`;
    invoice += 'Merci pour votre commande !\n';
    invoice += 'Votre commande sera prête dans 15-20 minutes.\n';
